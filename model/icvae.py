...
class Encoder(nn.Module):
    """ Encoder based on the model from "Accurate brain age prediction with lightweight deep neural networks",
    ...
    def __init__(self,
        ...
        self.pooling_kernel, self.pooling_stride = pooling_kernel, pooling_stride
        self.last_kernel, self.last_padding = last_kernel, last_padding
        ...
        for i in range(self.n_blocks):
            ...
            if i < self.n_blocks - 1:
                ...
                self.conv_blocks.append(conv_block(in_channels, self.channels[i], last_kernel, last_padding, stride))
...
class Decoder(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(self.n_blocks):
            ...
            if i == 0:
                self.tconv_blocks.append(tconv_block(self.channels[i], out_channels, first_kernel, first_padding,
                                                     stride))
            else:
                self.tconv_blocks.append(tconv_block(self.channels[i], out_channels, kernel_size, padding, stride))
        self.activation = nn.ReLU()
        self.unpooling_layer = nn.MaxUnpool3d(kernel_size=unpooling_kernel, stride=unpooling_stride)
...
class ICVAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = Encoder(input_shape, latent_dim, kernel_size, padding, stride, pooling_kernel, pooling_stride,
                               last_kernel, last_padding, channels)
        features_shape = self.encoder.features_shape(input_shape)
        channels.reverse()
        self.decoder = Decoder(features_shape, latent_dim, kernel_size, padding, stride, pooling_kernel, pooling_stride,
                               last_kernel, last_padding, channels)
